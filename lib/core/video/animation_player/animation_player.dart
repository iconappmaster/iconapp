...
import 'package:iconapp/core/video/animation_player/portrait_video_controls.dart';
...
class _AnimationPlayerState extends State<AnimationPlayer> {
  ...
  void initState() {
    super.initState();
    flickManager = FlickManager(
      videoPlayerController: VideoPlayerController.network(widget.urls[widget.index]),
      onVideoEnd: () => dataManager.playNextVideo(
        Duration(seconds: 5),
      ),
    );

    dataManager = AnimationPlayerDataManager(flickManager, widget.urls);
  }
