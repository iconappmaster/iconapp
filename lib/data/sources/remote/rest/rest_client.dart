...
abstract class RestClient {
  ...
  Future<Conversation> verifyConversationCode(
      @Path('conversationId') int conversationId, @Query('entranceCode') String code);
  ...
  Future deleteMessage(@Path('conversationId') int conversationId, @Query('messageId') int messageId);
  ...
  Future<Conversation> addUser(@Path('conversationId') int conversationId, @Query('userId') int userId);

  @POST('conversations/{conversationId}/remove_user')
  Future<Conversation> removeUser(@Path('conversationId') int conversationId, @Query('userId') int userId);
  ...
  Future<Conversation> getConversaion(@Path('conversationId') int conversationId,
  ...
  Future<Conversation> makeUserAdmin(@Path('conversationId') int id, @Query('userId') int userId);

  @POST('conversations/{conversationId}/set_pin')
  Future<Conversation> pinConversation(@Path('conversationId') int id, @Query('isPinned') bool isPinned);
  ...
  Future<List<MessageModel>> getCommentByConversationId(@Path('conversationId') int id);
  ...
  Future<Conversation> updateCommentSettings(
      @Path('conversationId') int id, @Query('commentsMaxUserCount') int commentsMaxUserCount);

  @POST('conversations/{conversationId}/send_comment')
  Future<MessageModel> sendComment(@Path('conversationId') int id, @Body() MessageModel comment);
  ...
  Future<MessageModel> likeMessage(@Path('messageId') int id, @Query('likeType') String likeType);
  ...
  Future<List<StoryModel>> homeStories(@Query('shouldShowAds') bool shouldShowAds);

  @GET('stories/stories_for_conversation')
  Future<List<StoryModel>> conversationStories(
      @Query('conversationId') int id, @Query('shouldShowAds') bool shouldShowAds);
  ...
  Future<StoryModel> addToStory(@Body() StoryModel story, @Query('numberOfHoursUntilExpiration') int duration);
